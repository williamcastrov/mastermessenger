...
function Pedidos() {
  ...
  const pedidoInsertar = (
    <div className={styles.modal}>
      ...
      <div align="right">
        <Button className={styles.button} color="primary" onClick={() => grabarPedido()} >Insertar</Button>
    ...
    <div className="App" >
      <div className={styles.modal2}>
        ...
        <Grid item xs={12} md={4}>
          <FormControl className={styles.formControl}>
            <InputLabel id="idselectid_cli">Cliente</InputLabel>
            <Select
              labelId="selectid_cli"
              name="id_cli"
              id="idselectid_cli"
              fullWidth
              onChange={handleChange}
              onClick={(e) => DatosClientes(e.target.value)}
            >
              <MenuItem value=""> <em>None</em> </MenuItem>
              {
                listarClientes && listarClientes.map((itemselect) => {
                  return (
                    <MenuItem value={itemselect.id_cli}>{itemselect.razonsocial_cli} {" "}{itemselect.primer_apellido_cli} </MenuItem>
                  )
                })
              }
            </Select>
        ...
        <div align="center">
          <Button className={styles.button} variant="contained" startIcon={<SaveIcon />} color="primary"
            onClick={() => abrirCerrarModalInsertar()}>  Crear Pedido
          </Button>
        </div>
      </div>
    </div>
