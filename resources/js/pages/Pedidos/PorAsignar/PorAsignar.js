...
function PorAsignar() {
  ...
  useEffect(() => {
    async function fetchDataPedidos() {
      const res = await pedidosServices.listpedidosporasignar();
      setListarPedidosPorServicio(res.data);
  ...
  useEffect(() => {
    async function fetchDataPedidos() {
      const res = await pedidosServices.listpedidosasignados();
      setListarPedidosAsignados(res.data);
      //console.log("PEDIDOS POR ASIGNAR :  ", res.data);
    }
    fetchDataPedidos();
  }, [])
  ...
  const seleccionarPedido = (pedidoasignar, caso) => {
    if (pedidoasignar.estado_ped === 2) {
      swal("Asignar Pedido", "Pedido seleccionado ya esta asignado!", "warning", { button: "Aceptar" });
      return
    }
    setPedidosSeleccionado(pedidoasignar);
    //console.log("PEDIDO ASIGNAR : ", pedidoasignar);
  ...
  const seleccionarPedidoDesAsignar = (desasignar, caso) => {
    ...
    setPedidosSeleccionado(desasignar);
    //console.log("PEDIDO DESASIGNAR : ", desasignar);
    (caso === "DesAsignar") ? abrirCerrarModalDesAsignar() : abrirCerrarModalEliminar()
  }
  ...
  const abrirCerrarModalDesAsignar = () => {
    setModalDesAsignar(!modalDesAsignar);
  }
  ...
  useEffect(() => {
    async function grabarMensajeroAsignado() {
      if (grabar) {
        //console.log("DATOS PEDIDO : ", pedidosSeleccionado[0].estado_ped);

        const res = await pedidosServices.update(pedidosSeleccionado[0]);

        if (res.success) {
          swal("Asignar Mensajero", "Mensajero asignado/retirado correctamente al pedido!", "success", { button: "Aceptar" });
          console.log(res.message);
          if (pedidosSeleccionado[0].estado_ped === 2)
            abrirCerrarModalAsignar();
          else
            abrirCerrarModalDesAsignar();
        } else {
          swal("Asignar Mensajero", "Error asignando/retirando mensajero al pedido!", "error", { button: "Aceptar" });
          console.log(res.message);
          if (pedidosSeleccionado[0].estado_ped === 2)
            abrirCerrarModalAsignar();
          else
          ...
          <ul>
            <Container>
              ...
              <Table>
                ...
                <tbody>
                  {listarPedidosPorServicio.map((item) => (
                    <tr key={item.id_ped}>
                      <td>{item.id_ped}</td>
  ...
  const pedidoDesAsignar = (
    <div className={styles.modal}>
      <div className="App">
        <Typography align="center" className={styles.typography} variant="button" display="block" >
          DESASIGNAR MENSAJERO A PEDIDO
        </Typography>
      </div>
      <Typography align="center" className={styles.typography2} variant="button" display="block" >
        {pedidosSeleccionado.id_ped}{"-"}{pedidosSeleccionado.razonsocial_cli}
      ...
      <Grid container spacing={2}>
          ...
          <ul>
            <Container>
              ...
              <Table>
                ...
                <tbody>
                  {listarPedidosAsignados.map((item) => (
                    <tr key={item.id_ped}>
                      <td>{item.id_ped}</td>
                      <td>{item.nombre_est}</td>
                      <td>{item.razonsocial_cli}</td>
        ...
        <Grid item xs={12} md={4}>
          <br />
          <form onSubmit={desAsignarMensajero} >
            <div >
              <Button className={styles.button} variant="contained" type="submit" size="small" color="secondary" >
                Retirar Mensajero
              </Button>
  ...
  return (
    <div className="App">
      ...
      <Modal
        open={modalDesAsignar}
        onClose={abrirCerrarModalDesAsignar}
      >
        {pedidoDesAsignar}
      </Modal>
